...
void feedforward(double input[NUM_INPUT_DIM], double output[NUM_OUTPUT_DIM], double w[NUM_HIDDEN][NUM_INPUT_DIM][NUM_INPUT_DIM], double x[NUM_HIDDEN][NUM_INPUT_DIM], double u[NUM_HIDDEN][NUM_INPUT_DIM]);
void backward(double input[NUM_INPUT_DIM], double output[NUM_OUTPUT_DIM], double d[NUM_OUTPUT_DIM],double w[NUM_HIDDEN][NUM_INPUT_DIM][NUM_INPUT_DIM], double v[NUM_INPUT_DIM][NUM_OUTPUT_DIM],
double x[NUM_HIDDEN][NUM_INPUT_DIM], double u[NUM_HIDDEN][NUM_INPUT_DIM]);
void filter(double **input,int input_height,int input_width, double **output, int step);
    ...
    for(int sample = 0; sample < K; sample++){
        ...
        for(int epoch = 0; epoch < num_epoch; epoch++){
            ...
            for(int data = 1; data <= NUM_TEST_DATA+NUM_TRAIN_DATA; data++) {
                ...
                test_err = 0.0;
                for (int i = 0; i < NUM_OUTPUT_DIM; ++i)
                    test_err += pow((test_d[i]-test_z[i]),2);
                
                test_err /= 2.0;
                test_overall_err[sample] += train_err;
...
int get_x(const char *filename, double train_x[NUM_INPUT_DIM])
{
    double tmp_image1[IMAGE_HEIGHT][IMAGE_WIDTH];
    double tmp_image2[IMAGE_HEIGHT/STEP][IMAGE_WIDTH/STEP];
    double tmp_image3[IMAGE_HEIGHT/STEP][IMAGE_WIDTH/STEP];
	...
	for ( i = 0; i < height; i = i + 10 ){
		for ( j = 0; j < width; j = j + 10 ) {
				tmp_image1[i][j] = (double)img[i][j]/255.0;
		}
	}
    conv_filter(tmp_image1,height,width,tmp_image2,STEP);
    pooling_filter(tmp_image2,height,width,tmp_image3,1);
...
void feedforward(double input[NUM_INPUT_DIM], double output[NUM_OUTPUT_DIM], double w[NUM_HIDDEN][NUM_INPUT_DIM][NUM_INPUT_DIM], double x[NUM_HIDDEN][NUM_INPUT_DIM], double u[NUM_HIDDEN][NUM_INPUT_DIM])
{
    ...
    for(int layer = 0; layer < NUM_HIDDEN; layer++) {//layer番目の隠れ層の
...
{
    double tmp  = 0.0;
    double delta_w[NUM_HIDDEN][NUM_INPUT_DIM];//エッジの重み,w[層][from][to]
    double delta_v[NUM_OUTPUT_DIM];//出力層の重み
    double grad_w[NUM_HIDDEN][NUM_INPUT_DIM][NUM_INPUT_DIM];//エッジの重み,w[層][from][to]
    double grad_v[NUM_INPUT_DIM][NUM_OUTPUT_DIM];//出力層の重み
    ...
    //隠れ層のdeltaを求める
    for (int layer = NUM_HIDDEN - 1; layer >= 0; layer--){
        if(layer == NUM_HIDDEN - 1){
            ...
            for(int to = 0; to < NUM_INPUT_DIM; to++) {
                    for (int i = 0; i < NUM_INPUT_DIM - 1; ++i){
    ...
    for (int layer = 0; layer < NUM_HIDDEN; ++layer){
        if (layer == 0){
            for (int to = 0; to < NUM_INPUT_DIM - 1; ++to){
                for (int from = 0; from < NUM_INPUT_DIM; ++from){
                    if (from != NUM_INPUT_DIM - 1) {
                        grad_w[layer][from][to] =  input[from] * delta_w[layer][to];
                        norm_grad += pow(grad_w[layer][from][to] ,2);
    ...
    for (int to = 0; to < NUM_OUTPUT_DIM; ++to){
        for (int from = 0; from < NUM_INPUT_DIM; ++from){
            if (from != NUM_INPUT_DIM - 1) {
                grad_v[from][to] = x[NUM_HIDDEN - 1][from] * delta_v[to];
                norm_grad += pow(grad_v[from][to] ,2); 
            }else {
    ...
    for (int layer = 0; layer < NUM_HIDDEN; ++layer){
        for (int to = 0; to < NUM_INPUT_DIM - 1; ++to){
            for (int from = 0; from < NUM_INPUT_DIM; ++from){
                w[layer][from][to] -= eta * grad_w[layer][from][to] / norm_grad; 
            }
        }
    }
    ...
    for (int to = 0; to < NUM_OUTPUT_DIM; ++to){
        for (int from = 0; from < NUM_INPUT_DIM; ++from){
            v[from][to] -= eta * grad_v[from][to] / norm_grad; 
        }
    }

}

void conv_filter(double **input,int input_height,int input_width ,double **output, int step)
{
    ...
    int height_count = 0;
    int width_count = 0;
    for(size_t height = 0; height < input_height; height++){
        for(size_t width = 0; width < input_width; width++){
            for(size_t i = 0; i < WINDOW_SIZE; i = i + step){
                for(size_t j = 0; j < WINDOW_SIZE; j = j + step){
                    output[height_count][width_count] = input[height - 1 + i][width - 1 + j] * window[i][j]
                }
            }
        }
    }
...
void pooling_filter(double **input,int input_height,int input_width ,double **output, int step)
{
    int height_count = 0;
    int width_count = 0;
    for(size_t height = 0; height < input_height; height++){
        for(size_t width = 0; width < input_width; width++){
            double max = 0.0;
            for(size_t i = 0; i < 3; i = i + step){
                for(size_t j = 0; j < 3; j = j + step){
                    if( input[height - 1 + i][width - 1 + j]  >= max) max =  input[height - 1 + i][width - 1 + j]; 
                }
            }
            output[height_count][width_count] = max;
        }
    }
}