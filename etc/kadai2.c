            ...
            for(int data = 1; data <= NUM_TEST_DATA+NUM_TRAIN_DATA; data++) {
                ...
                for (layer = 0; layer < NUM_HIDDEN; ++layer){
                    if (layer == 0){
                        for (int to = 0; to < NUM_INPUT_DIM; ++to){
                            for (int from = 0; from < NUM_INPUT_DIM; ++from){
                                norm += pow(train_x[from] * delta_w[layer][from][to],2); 
                            }
                        }
                    }else{
                        for (int to = 0; to < NUM_INPUT_DIM; ++to){
                            for (int from = 0; from < NUM_INPUT_DIM; ++from){
                                norm  += pow(x[layer - 1][from] * delta_w[layer][from][to] , 2); 
                            }
                        }
                    }
                }
                for (int to = 0; to < NUM_OUTPUT_DIM; ++to){
                    for (int from = 0; from < NUM_INPUT_DIM; ++from){
                        norm += pow(x[NUM_HIDDEN - 1][from] * delta_v[from][to],2); 
                    }
                }
                norm = pow(norm , 0.5);
                ...
                for (layer = 0; layer < NUM_HIDDEN; ++layer){
                    if (layer == 0){
                        for (int to = 0; to < NUM_INPUT_DIM; ++to){
                            for (int from = 0; from < NUM_INPUT_DIM; ++from){
                                w[layer][from][to] -= eta * train_x[from] * delta_w[layer][from][to] / norm; 
                            }
                        }
                    }else{
                        for (int to = 0; to < NUM_INPUT_DIM; ++to){
                            for (int from = 0; from < NUM_INPUT_DIM; ++from){
                                w[layer][from][to] -= eta * x[layer - 1][from] * delta_w[layer][from][to] / norm; 
                            }
                        }
                    }
                }
                ...
                for (int to = 0; to < NUM_OUTPUT_DIM; ++to){
                    for (int from = 0; from < NUM_INPUT_DIM; ++from){
                        v[from][to] -= eta * x[NUM_HIDDEN - 1][from] * delta_v[from][to] / norm; 
                    }
                }
                train_err = 0.0;
                //損失を計算
