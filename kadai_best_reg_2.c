...
{ 
    ...
    for(int sample = 0; sample < K; sample++){
        ...
        for(int i=0; i<NUM_LAYER_DEPTH; i++) {
            for(int j=0; j<NUM_LAYER_DIM; j++) {
                for (int k = 0; k < NUM_LAYER_DIM; ++k){
        ...
        for(epoch = 1; epoch < num_epoch; epoch++){
            //教師データで学習
            for(int data = 1; data <= NUM_TEST_DATA+NUM_TRAIN_DATA; data++) {
                ...
                //printf("data %d ,epoch:%d train_err:%lf\n",data,epoch,train_err);
            ...
            //print detail
            printf("sample :%d, epoch %d ,test_overall_err:%lf ||  train_overall_err:%lf\n",sample,epoch,test_overall_err[sample]/(double)NUM_TEST_DATA,train_overall_err);
        }
...
void feedforward(double input[NUM_INPUT_DIM], double output[NUM_OUTPUT_DIM], double w[NUM_LAYER_DEPTH][NUM_LAYER_DIM][NUM_LAYER_DIM], double x[NUM_LAYER_DEPTH][NUM_LAYER_DIM], double u[NUM_LAYER_DEPTH][NUM_LAYER_DIM])
{
    ...
    //隠れ層の計算
    for(int i=0; i< NUM_LAYER_DEPTH; i++) {
        for(int j=0; j < NUM_LAYER_DEPTH; j++) {
            x[i][j] = 0.0;
            u[i][j] = 0.0;
        }
    }
    for(int layer = 0; layer < NUM_LAYER_DEPTH; layer++) {//layer番目の隠れ層の
        for (int to = 0; to < NUM_LAYER_DIM; to++){//m番目の次元の計算
...
double x[NUM_LAYER_DEPTH][NUM_LAYER_DIM], double u[NUM_LAYER_DEPTH][NUM_LAYER_DIM])
{
    ...
    static double m_w[NUM_LAYER_DEPTH][NUM_LAYER_DIM][NUM_LAYER_DIM];//エッジの重み,w[層][from][to]
    static double m_v[NUM_LAYER_DIM][NUM_OUTPUT_DIM];//出力層の重み
    static double v_w[NUM_LAYER_DEPTH][NUM_LAYER_DIM][NUM_LAYER_DIM];//エッジの重み,w[層][from][to]
    static double v_v[NUM_LAYER_DIM][NUM_OUTPUT_DIM];//出力層の重み
        ...
        if (layer == 0){
            for (int to = 0; to < NUM_LAYER_DIM - 1; ++to){
                for (int from = 0; from < NUM_LAYER_DIM; ++from){
                    ...
                    m_w[layer][from][to] += beta1*m_w[layer][from][to]+(1-beta1)*grad_w[layer][from][to];
                    v_w[layer][from][to] += beta2*v_w[layer][from][to]+(1-beta2)*grad_w[layer][from][to] * grad_w[layer][from][to];
    ...
    for (int to = 0; to < NUM_OUTPUT_DIM; ++to){
        for (int from = 0; from < NUM_LAYER_DIM; ++from){
            ...
            m_v[from][to] += beta1*m_v[from][to]+(1-beta1)*grad_v[from][to];
            v_v[from][to] += beta2*v_v[from][to]+(1-beta2)*grad_v[from][to] * grad_v[from][to];
        }
    }
    ...
    for (int layer = 0; layer < NUM_LAYER_DEPTH; ++layer){
        for (int to = 0; to < NUM_LAYER_DIM - 1; ++to){
            for (int from = 0; from < NUM_LAYER_DIM; ++from){
                w[layer][from][to] -= eta * (m_w[layer][from][to]/(1 - pow(beta1,epoch)) ) * grad_w[layer][from][to] / ( pow( v_w[layer][from][to] /  (1.0 - pow(beta2,epoch) ) ,0.5) + EPSILON); 
            }
        }
    }
    ...
    for (int to = 0; to < NUM_OUTPUT_DIM; ++to){
        for (int from = 0; from < NUM_LAYER_DIM; ++from){
            v[from][to] -= eta *(m_v[from][to]/(1 - pow(beta1,epoch) ) ) *grad_v[from][to] / ( pow( v_v[from][to] /  (1.0 - pow(beta2,epoch) ) ,0.5) + EPSILON); 
        }
    }
    count++;
