//パターン認識プログラミング課題 ニューラルネット
//学籍番号:201611212,氏名:渡部祐也
#include "stdio.h"
#include "math.h"
#include "time.h"
#include "stdlib.h"
...
#define NUM_INPUT_DIM 12*15 + 1 //120/10*150/10 + 1 画像サイズは120*150だが、そのまま使うとメモリに乗らないため、適当に1/100に圧縮して使いました。 
#define NUM_OUTPUT_DIM 1//出力次元
#define NUM_LAYER_DIM 12*15 + 1
#define NUM_LAYER_DEPTH 1
const int  NUM_TRAIN_DATA  = 496;
#define NUM_TEST_DATA 124
#define  K  5//交差検証で分割するクラス数
...
{ 
    ...
    for(int sample = 0; sample < K; sample++){
        ...
        for(int epoch = 0; epoch < num_epoch; epoch++){
            ...
            test_overall_err[sample] /= (double)NUM_TEST_DATA;
            //-----------------------------------途中経過をprint---------------------------------
            printf("sample :%d, epoch %d ,train_overall_err:%lf ||  test_overall_err:%lf\n",sample,epoch,train_overall_err,test_overall_err[sample]);
        }       
    }

    //--------------------------------------------------------------
    double ave_test_err = 0.0;
    for(size_t i = 0; i < K; i++){
        ave_test_err += test_overall_err[i];
    }
    ave_test_err /= (double)K;
    //-----------------------------------------------------------------
    ...
    printf("--------------------------------Result--------------------------------------\n");    
    for(size_t i = 0; i < K; i++){
        printf("test set %d : test_err %lf\n",i,test_overall_err[i]);
    }
    printf("average test err : %lf\n",ave_test_err);
    return 0;
}
...
double x[NUM_LAYER_DEPTH][NUM_LAYER_DIM], double u[NUM_LAYER_DEPTH][NUM_LAYER_DIM])
{
    ...
    for (int layer = 0; layer < NUM_LAYER_DEPTH; ++layer){
        for (int to = 0; to < NUM_LAYER_DIM - 1; ++to){
            for (int from = 0; from < NUM_LAYER_DIM; ++from){
                w[layer][from][to] -= eta * grad_w[layer][from][to] ; 
            }
        }
    }
